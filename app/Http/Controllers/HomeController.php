...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('home.index')->with(['polls' => $polls,'pollsfinalizados' => $pollsfinalizados, 'pollsnaoiniciadas'=>$pollsnaoiniciadas]);
    }

    public function show(Poll $home)
    {
        $options = Option::where('polls_id', '=',$home->id)->orderBy('id','asc')->get();
        $total = Option::where('polls_id','=',$home->id)->sum('qtd_votos');
    ...
    public function update(Request $home)
    {
        if (is_null($home->input('opcao'))) {
            ...
            $opcao = Option::find($home->input('opcao'));
            $opcao->qtd_votos++;
            $opcao->save();
        }
        return back();
